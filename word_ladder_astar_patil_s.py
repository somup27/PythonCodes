...
def a_star(word_list, start, goal):
    ...
    while frontier:
        currNode = frontier.pop() # Pop the node with the smallest cost+heuristic value
        if len(currNode[1]) != 0 and currNode[2] == goal: # Checking if the goal has been
            return currNode[1]
        else:
            for child in generate_adjacents(currNode,words): # Generates all the children
                words.remove(child)
                frontier.append((dist_heuristic(currNode[2],goal)+1+diction[currNode[2]],currNode[1]+[child],child))
                diction[child] = diction[currNode[2]]+1
    return None
...
def main():
    ...
    cur_time = time.time()
    path_and_steps = (a_star(words, initial, goal))
    rev = a_star(word_list,goal,initial)
    if len(rev)<len(path_and_steps):
        path_and_steps = rev[::-1]
